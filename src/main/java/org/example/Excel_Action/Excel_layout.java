...
public class Excel_layout {
    public static void edit_layout(XSSFWorkbook workbook, XSSFSheet sheet, int counter) {
        //edit column width
        for (int x = 0; x <= 9; x++) {
            sheet.setColumnWidth(x, excel_setting.layout_message_width);
        }
        sheet.setColumnWidth(0, excel_setting.layout_variable_width);
        sheet.setColumnWidth(1, excel_setting.layout_variable_width);
        //set first row title
